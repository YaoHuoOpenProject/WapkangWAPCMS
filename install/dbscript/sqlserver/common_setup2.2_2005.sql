...
AS
	...
	IF @pageindex = 1
		SELECT TOP 10 [seqid],[code],[typeid],[createdtime],[creator],[creatorid],[expiretime],[maxcount],[successcount] FROM [wapkang_invitation] WHERE [creatorid]=@creatorid AND [typeid]=3 AND [isdeleted]=0 ORDER BY [seqid] DESC
	ELSE
	BEGIN
...
BEGIN
	...
	BEGIN
        SET @ResultSql = 'SELECT * FROM (SELECT ROW_NUMBER() Over(ORDER BY '+ @OrderField +' '+@AOrderString+') AS TEMPROWID,' + @FilterColumn + ' FROM ' + @TableName + ' WHERE ' + @FilterCondition 
	END
	ELSE
	BEGIN
        SET @ResultSql = 'SELECT * FROM (SELECT ROW_NUMBER() Over(ORDER BY '+ @OrderField +' '+@AOrderString+') AS TEMPROWID,' + @FilterColumn + ' FROM ' + @TableName
	END
    SET @ResultSql = @ResultSql + ') AS [temp] WHERE [TEMPROWID] BETWEEN ' + Convert(varchar(50),@StartRecord) + ' AND ' +  Convert(varchar(50),@EndRecord) + ' ORDER BY [TEMPROWID] '+@BOrderString
	Print @ResultSql
	EXEC(@ResultSql)
END
GO
...
[TEMP].[Isavatar],
[TEMP].[Vipcredits]
FROM(SELECT ROW_NUMBER() OVER(ORDER BY [Seqid]) AS ROWID,
[u].[Seqid], 
[u].[Nickname], 
[u].[Username], 
[u].[Gender], 
...
WHERE [ui].[tagid] = @interestid) AS [TEMP]
